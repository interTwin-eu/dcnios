    ...
    if "components" in type:
        ...
        for component in type["components"]:
            ...
            if "seconds" in component:
                nifi.changeSchedule(type["name"], component["name"],
                                    component["seconds"])
                print("\t  New schedule time: "
                      + str(component["seconds"]) + " seconds")
            if "node" in component:
                nifi.executionNode(type["name"],
        ...
        if args.option == "apply":
            if "dCache" in data["nifi"]:
                for dcache in data["nifi"]["dCache"]:
                    ...
                    command = "simple-client.py --state /state/" \
                        + dcache["statefile"] + " --endpoint " \
                        + dcache["endpoint"] \
                        + " --user "+dcache["user"] + " --password " \
            ...
            if "OSCAR" in data["nifi"]:
                for oscar in data["nifi"]["OSCAR"]:
                    ...
                    nifi.changeVariable(oscar["name"], "endpoint",
                                        oscar["endpoint"])
                    nifi.changeVariable(oscar["name"], "service",
                                        oscar["service"])
                    if "user" in oscar and "password" in oscar:
                        client = Client("cluster-id", oscar["endpoint"],
                                        oscar["user"], oscar["password"], True)
                        service = client.get_service(oscar["service"])
                        token = service.json()["token"]
                        nifi.changeVariable(oscar["name"], "token", token)
                    else:
                        nifi.changeVariable(oscar["name"], "token",
                                            oscar["token"])
            ...
            if "generic" in data["nifi"]:
                for generic in data["nifi"]["generic"]:
                    ...
                    for variable in generic["variables"]:
                        nifi.changeVariable(generic["name"], variable,
            ...
            if "SQS" in data["nifi"]:
                for sqs in data["nifi"]["SQS"]:
                    ...
                    nifi.changeVariable(sqs["name"], 'queueurl',
                                        sqsDetails['QueueUrl'])
            ...
            if "S3" in data["nifi"]:
                for s3 in data["nifi"]["S3"]:
                    ...
                    nifi.changeVariable(s3["name"], 'queueurl',
                                        sqsDetails['QueueUrl'])
            ...
            if "Kafka" in data["nifi"]:
                for kafka in data["nifi"]["Kafka"]:
                    ...
                    kafka["ssl_context"]["Truststore_Filename"] = "/opt/nifi" \
                        + "/nifi-current/data/" \
                        + kafka["ssl_context"]["Truststore_Filename"]
                    kafkacontent = ssl_context(kafkacontent,
                                               kafka["ssl_context"])
                    # Create object
                    nifi.create(kafka["name"], kafkacontent)
                    nifi.changeVariable(kafka["name"], "group_id",
                                        kafka["group_id"])
                    nifi.changeVariable(kafka["name"], "bootstrap_servers",
                                        kafka["bootstrap_servers"])
                    nifi.changeVariable(kafka["name"], "topic",
                                        kafka["topic"])
                    # Enable SSL
